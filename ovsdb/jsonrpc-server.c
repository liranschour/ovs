...
ovsdb_jsonrpc_monitor_compose_update(struct ovsdb_jsonrpc_monitor *m,
                                     bool initial)
{
    bool cond_updated = m->cond_updated;

    if (!cond_updated && !ovsdb_monitor_needs_flush(m->dbmon, m->unflushed)) {
        return NULL;
    }

    m->cond_updated = false;
    return ovsdb_monitor_get_update(m->dbmon, initial, cond_updated,
...
ovsdb_jsonrpc_monitor_needs_flush(struct ovsdb_jsonrpc_session *s)
{
    ...
    HMAP_FOR_EACH (m, node, &s->monitors) {
        if (m->cond_updated || ovsdb_monitor_needs_flush(m->dbmon,
                                                         m->unflushed)) {
            return true;
        }
    }
