...
enum monitor_table_condition_mode {
    ...
    MTC_MODE_EQ_COND, /* only "==" conditional monitoring */
 ...
 * Tracked by table's changes OR by ovsdb_tracked_clauses */
struct ovsdb_monitor_row {
    ...
    struct ovs_list old_list;   /* List of Old data sorted by transaction */
                         ...
                         bool monitored)
{
    ...
    /* Check duplication only for none monitored columns.
     * We do not remove unused none monitored columns due to condition
     * update */
    if (!monitored) {
        int i;
        for (i = 0; i < mt->n_columns; i++) {
                          ...
                          struct ovsdb_monitor_session_condition *cond)
{
    ...
    SHASH_FOR_EACH(node, &cond->tables) {
        ...
        if (mtc->cond_mode == MTC_MODE_EQ_COND) {
            ...
            mt->track_clauses = true;
            ovsdb_monitor_condition_update_tracking(mtc->mt,
                                                    &mtc->old_condition,
                                                    NULL);
            /* Track the next unflushed txn */
            ovsdb_monitor_table_track(mt, mt->dbmon->n_transactions + 1);
        }
    }
}
                                ...
                                uint64_t next_txn)
{
    ...
    changes->mt = mt;
    changes->n_refs = 1;
    ovsdb_datum_init_empty(&changes->arg);
...
/* Save processed rows to avoid duplications if row matchs more the one clause */
struct row_uuid {
                      ...
                      enum ovsdb_monitor_version version)
{
    ...
    SHASH_FOR_EACH (node, &dbmon->tables) {
        ...
        if (!mtc || mtc->cond_mode == MTC_MODE_TRUE) {
            cache_node = ovsdb_monitor_json_cache_search(&mt->json_cache, version, transaction);
        }
        if (cache_node) {
            table_json = cache_node->json ? json_clone(cache_node->json) : NULL;
            if (table_json) {
                if (!json) {
                    json = json_object_create();
                }
            ...
            if (!initial && mtc && mtc->cond_mode == MTC_MODE_EQ_COND) {
                ...
                for (i = 0; i < mtc->old_condition.n_clauses; i++) {
                    struct ovsdb_clause *clause = &mtc->old_condition.clauses[i];

                    changes = ovsdb_monitor_table_find_clause_changes(mtc, clause);
                    if (!changes) {
                        continue;
                    }
                ...
                /* Update tracking of transaction ID */
                ovsdb_monitor_table_untrack(mt, transaction);
                ovsdb_monitor_table_track(mt, mt->dbmon->n_transactions + 1);
                ...
                if (!mtc || !mtc->cond_mode) {
                    ovsdb_monitor_json_cache_insert(&mt->json_cache, version, transaction, table_json);
                }
            }
        }
    }
                             ...
                             uint64_t transaction)
{
    ...
    if (!change) {
        struct ovsdb_datum *old_data = clone_monitor_row_data(mt, old);
                        ...
                        void *aux_)
{
    ...
    if (efficacy > OVSDB_CHANGES_NO_EFFECT) {
        ...
        if (mt->track_clauses) {
            ovsdb_monitor_clauses_track_update(old, new, mt);
        }
    }
