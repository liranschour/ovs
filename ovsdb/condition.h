...
struct ovsdb_condition {
    ...
    size_t n_clauses;
    /* bitmap of all the OVSDB_F_* included in condition */
    unsigned long int functions[BITMAP_N_LONGS(OVSDB_F_LAST)];
};

#define OVSDB_CONDITION_INITIALIZER { NULL, 0, {0}}
...
static inline bool
ovsdb_condition_includes_functions(const struct ovsdb_condition *cnd,
                                   unsigned long int functions[])
{
    bitmap_and(functions, cnd->functions, OVSDB_F_LAST);
    return bitmap_count1(functions, OVSDB_F_LAST);
}
